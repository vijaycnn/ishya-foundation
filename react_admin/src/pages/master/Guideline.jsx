...
function Guideline() {
  ...
  const slidePageNumber = 'guideline';
  ...
  const updateMedia = async (mediaId, fileUrl)=>{
    ...
    const fileUploadEvent = (file) => {
      ...
      console.log("fileType", selected.type, formData);
        if (!allowedImgTypes.includes(selected.type)) {
            setFileError(
            "Only PDF, DOCX, JPEG, PNG, JPG, WEBP files are allowed."
            );
            setUploadMediaFile(null);
            return;
        }
        if (selected.size > MAX_IMAGE_SIZE_LBL) {
    ...
    const validation = (values) => {
        ...
        if(uploadMediaFile){
            ...
            if (uploadMediaFile.size > MAX_IMAGE_SIZE_LBL) {
                setFileError(`File size must be less than ${MAX_IMAGE_SIZE_LBL}.`);
                hasError = true;
            }          
        }
                    ...
                    <td>      
                      {
                          ...
                          <>
                              ...
                              > */}
                                <a href={item.filePath} target="_blank" className="rounded-pill" >
                                  <span>
                                    View File
                                  </span>
                                </a>
  ...
  return (
    <>
      <h1 className="h4 mb-4 font-secondary fw-medium">Upload Guideline</h1>
      <div className="table-view bg-white rounded-4 p-4">
          ...
          <div>
            {
                ...
                <Link onClick={ ()=>openAdd()} className="btn btn-primary btn-sm">
                    <span className="nav-link-text">Add-File</span>
                </Link>
                </> : ''
            }              
          </div>
        ...
        <Modal id="frm" name="frm" size="" show={show} centered onHide={() => setShow(false)} backdrop="static" keyboard={false}>
            <Modal.Header closeButton>
                <Modal.Title>
                    {mode === "add" ? "Add Guideline" : "Edit Guideline"}
                </Modal.Title>
            </Modal.Header>
            <Modal.Body>
                {/* <Form.Group > */}
                ...
                {successMsg && <Alert variant="success">{successMsg}</Alert>}
                    {mode === "edit" && previewUrl && (
                      <div className="mb-3 text-center">
                        ...
                        <a href={previewUrl} target="_blank" className="rounded-pill" >
                          <span>View File</span>
                        </a>
