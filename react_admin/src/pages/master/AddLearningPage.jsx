...
function AddLearningPage() {
  ...
  const validation = (values) => {
    setError("");
    let hasError = false;
    if (!values.type || values.type == "" || !values.title || values.title == "" || !values.remark1 || values?.remark1.trim() == "" || values.orderNumber == "" || !values.orderNumber || uploadMediaFile == null) {
      ...
      await axiosInstance
        ...
        .then((response) => {
          ...
          if (response.data.status === "success") {
            ...
            setLoading(false);
            setTimeout(() => {
              navigate(`${adminAlias}/learningpages`);
            }, 2000);
          } else if (response.data.status === "error") {
            setError(response.data.message);
          }
  ...
  return (
    <>
      ...
      <div className="mb-3 d-flex justify-content-between align-items-center">
        <h1 className="h4 mb-0 font-secondary fw-medium">Add Learning Page</h1>
        <div>
          <Link to={`${adminAlias}/learningpages`} className="btn btn-primary btn-sm">
