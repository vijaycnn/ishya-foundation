      ...
      <div className="grid-container">
        ...
        <div className="row first-row">
          {images.slice(0, 3).map((src, index) => (
            <img key={index} src={media(src)} alt={`Team Member ${index + 1}`} className="grid-image large" loading="lazy"/>
          ))}
        </div>
        ...
        <div className="row second-row">
          {images.slice(3, 7).map((src, index) => (
            <img key={index + 3} src={media(src)} alt={`Team Member ${index + 4}`} className="grid-image small"loading="lazy" />
          ))}
        </div>
      </div>
    </div>
